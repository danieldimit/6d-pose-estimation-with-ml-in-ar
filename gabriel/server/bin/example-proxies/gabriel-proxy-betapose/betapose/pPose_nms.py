...
def write_json(all_results, outputpath, for_eval=False):
    ...
    for im_res in all_results:
        ...
        for human in im_res['result']:
            ...
            if len(cam_R) > 0:
                result['cam_R'] = np.array(cam_R).reshape((9, 1))[:, 0].tolist()
                result['cam_t'] = np.array(cam_t).reshape((3, 1))[:, 0].tolist()
            ...
            if form == 'cmu': # the form of CMU-Pose
                if result['image_id'] not in json_results_cmu.keys():
                    ...
                    json_results_cmu[result['image_id']]['version']="Betapose v1.0"
                ...
                result['keypoints'].append((result['keypoints'][50]+result['keypoints'][20])/2)
                ...
                if result['image_id'] not in json_results_cmu.keys():
                    json_results_cmu[result['image_id']]={}
                    json_results_cmu[result['image_id']]['version']="Betapose v0.2"
    ...
    if form == 'cmu': # the form of CMU-Pose
        with open(os.path.join(outputpath,'Betapose-results.json'), 'w') as json_file:
    ...
    print("Results have been written to", os.path.join(outputpath,'Betapose-results.json'))
