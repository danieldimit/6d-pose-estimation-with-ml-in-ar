...
def process_command_line(argv):
    ...
    '''
    VERSION = gabriel3.Const.VERSION
...
def print_rtn(rtn_json):
    ...
    '''
    if gabriel3.Protocol_client.JSON_KEY_RESULT_MESSAGE in rtn_json:
        print_json = dict(rtn_json)
        result_str = print_json[gabriel3.Protocol_client.JSON_KEY_RESULT_MESSAGE]
        result_json = json.loads(result_str)
        image_str = result_json.get(gabriel3.Protocol_result.JSON_KEY_IMAGE, None)
        if image_str is not None:
            result_json[gabriel3.Protocol_result.JSON_KEY_IMAGE] = "an image of %d bytes" % len(image_str)
        result_str = json.dumps(result_json)
        print_json[gabriel3.Protocol_client.JSON_KEY_RESULT_MESSAGE] = result_str
    else:
        print_json = rtn_json
